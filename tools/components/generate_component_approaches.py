...
def write_component_files(characters, ids_dict, similar_idx=None, mma_dict=None, memo_dict=None, output_dir='.'):
    ...
    def _final_norm(x):
        x = _RADICAL_NORM.get(x, x)
        if x == '𫩠': return '尚'
        if x == '龸': return '尚'
        if x == '&CDP-8958;': return '月'
        return x
    def _final_ok(x):
        x = _final_norm(x)
        return x == '⺌' or _ok(x)
    ...
    for c in characters:
        ...
        for comp in final_direct[c]:
            expanded = [comp]
            if comp not in character_set:
                children = [_final_norm(child) for child in _direct_list(comp) if _final_ok(child) and child != c]
                if children: expanded = children
            for child in expanded:
                if child != c and child not in seen:
                    seen.add(child)
                    ordered += [child]
        final_ordered[c] = ordered
    final_map = {c: set(final_ordered[c]) for c in characters}
    final_core = {}
    for c in characters:
        disc = _discriminating(c, final_map)
        final_core[c] = [ch for ch in final_ordered[c] if ch in disc]
        if final_direct[c] and not final_core[c]:
            final_core[c] = final_direct[c]

    files = [('data_components_direct.json', direct), ('data_components_all.json', all_recursive), ('data_components_hanzi.json', hanzi), ('data_components_rare.json', rare)]
    ...
    for c in characters:
        ...
        for comp in family[c]:
            ...
            if comp != c and _final_ok(comp) and comp not in seen:
                seen.add(comp)
                ordered += [comp]
        family_final[c] = ordered
    final = {}
    for c in characters:
        target = set(family_final[c]) | set(final_core[c])
        seen, ordered = set(), []
        for comp in final_direct[c]:
            if comp in target and comp not in seen:
                seen.add(comp)
                ordered += [comp]
            if comp in character_set: continue
            children = [_final_norm(child) for child in _direct_list(comp) if _final_ok(child) and child != c]
            for child in children:
                if child in target and child not in seen:
                    seen.add(child)
                    ordered += [child]
...
if __name__ == "__main__":
    ...
    OUT = DATA_DIR / "derived" / "components" / "approaches"
    ...
    print("\nWriting non-final approach component files...")
