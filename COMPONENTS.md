...
- **Final** — the card-targeted final approach. Combine `family` with a normalized depth-1 direct decomposition, then order the result by structural appearance instead of forcing `family` first. Keep direct chunks by default rather than recursively unwrapping them, expand only nested top-level IDS child objects from `ids_dictionary.json` when the target character itself has a nested direct child (for cases like 光), and normalize learner-facing forms such as `⺌→小`, `⺮→竹`, `𧾷→足`, `𤣩→王`, `礻→示`, `⻏/⻖→阝`, `𫩠/龸→尚`, and `&CDP-8958;→月`. This preserves the direct structural chunk, keeps shared confusion-group context, and avoids dataset-membership-driven unwrapping, mixed granularity, and partial-loss stumps: 肖=`['小','月']`; 数=`['娄','攵']`; 痛=`['疒','甬']`; 幅=`['巾','畐']`; 常=`['尚','巾']`; 路=`['足','各']`.
