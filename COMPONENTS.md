...
- **Final** — the card-targeted final approach. Combine `family` with a normalized depth-1 direct decomposition, then order the result by structural appearance instead of forcing `family` first. Keep direct chunks by default rather than recursively unwrapping them, expand nested top-level IDS child objects only through the current explicit direct-view logic, and normalize learner-facing forms such as `⺌→小`, `⺮→竹`, `𧾷→足`, `𤣩→王`, `礻→示`, `牜→牛`, `𠆢→人`, `⻏/⻖→阝`, `𫩠/龸→尚`, and `&CDP-8958;→月`. Also allow a small explicit self-override list for repeated-stack mnemonics and high-value opaque singletons: `众=['众']`, `森=['森']`, `晶=['晶']`, `六=['六']`, `只=['只']`, `直=['直']`, `南=['南']`, `北=['北']`, `带=['带']`, `争=['争']`, `单=['单']`, `曾=['曾']`, `尔=['尔']`. The embedded host-character override map handles the repaired opaque families and explicit singleton splits. Current explicit replacements are: `师=['刂','帀']`, `归=['刂','彐']`, `帅=['刂','巾']`, `候=['亻','矦']`, `侯=['亻','矦']`, `留=['卯','田']`, `贸=['卯','贝']`, `即=['皀','卩']`, `既=['皀','旡']`, `辨=['辡','刂']`, `班=['玨','刂']`, `介=['人','八']`, `养=['羊','介']`, `乔=['夭','八']`, `其=['甘','八']`, `以=['以']`, `可=['丁','口']`, `那=['冄','阝']`, `发=['发']`, `原=['厂','泉']`, `武=['戈','止']`, `局=['尸','句']`, `摇=['扌','䍃']`, `典=['冊','廾']`, `延=['廴','正']`, `然=['肰','灬']`, `或=['戈','口']`, `制=['制']`, `唐=['广','口','廿']`, `丝=['丝']`, `象=['象']`, `与=['与']`, `度=['广','廿','又']`, `展=['展']`, `参=['参']`, `段=['段']`, `印=['印']`, `益=['益']`, `夜=['夜']`, `疑=['疑']`, `免=['免']`, `款=['柰','欠']`, `旅=['旅']`, `奥=['奥']`, `舞=['舞']`, `拜=['手','𠦪']`, `曼=['曼']`, `乌=['乌']`, `梁=['氵','刅','木']`, `妻=['十','女']`, `席=['广','巾','廿']`, `兼=['兼']`, `肃=['肃']`, `惠=['叀','心']`, `曹=['曹']`, `饰=['饣','巾']`, `寿=['寿']`, `匆=['匆']`, `衰=['衰']`, `燕=['燕']`, `祭=['祭']`, `函=['函']`, `疆=['弓','畺','土']`, `卑=['卑']`, `伞=['人','十','丷']`, `脊=['脊']`, `淫=['氵','爫','士']`, `舆=['舁','车']`, `鼎=['鼎']`, `焉=['正','灬']`, `詹=['厃','言']`, `勿=['勿']`, `甩=['甩']`, `嗣=['冊','司']`, `顷=['匕','页']`, `隋=['隋']`, `农=['农']`, `丧=['丧']`, `畏=['畏']`, `产=['产']`, `商=['立','冏']`, `受=['爫','又']`, `爱=['冖','友']`, `帝=['立','巾']`, `旁=['立','方']`, `囊=['口','襄']`, `襄=['衣','口']`, `在=['才','土']`, `存=['才','子']`, `岛=['鸟','山']`, and `鸟=['鸟']`. Keep the next second-tier coarse candidates as self-components for now: `更`, `果`, `我`, `身`, `音`, `言`, `亦`, `谷`, `辛`, `鬼`, `鱼`, `龙`, `甫`, `辰`, `弗`, `夹`. When the computed list would otherwise be empty, `final` falls back to `[char]`, so `components.json` now has 2998/2998 non-empty entries and no remaining CDP/entity placeholders. This preserves the direct structural chunk, keeps shared confusion-group context, and avoids dataset-membership-driven unwrapping, mixed granularity, and partial-loss stumps from the old direct-child recursion: 肖=`['小','月']`; 数=`['娄','攵']`; 痛=`['疒','甬']`; 幅=`['巾','畐']`; 常=`['尚','巾']`; 路=`['足','各']`.
